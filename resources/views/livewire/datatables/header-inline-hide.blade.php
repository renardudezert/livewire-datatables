...
<div @isset($column['headerAttributes']) @foreach ($column['headerAttributes'] as $attr => $val) {{$attr}}="{{ $val }}" @endforeach @endisset 
    class="@if($column['hidden']) hidden @else relative h-12 overflow-hidden align-top flex table-cell @endif" @include('datatables::style-width')>
